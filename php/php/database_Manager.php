...
class database_Manager{
    ...
    /*public function checkEventiDate($eventi){
        ...
        foreach($eventi as $evento){
            if($evento['data'] <  $date || $evento['data'] > $date){
    ...
    */
    ...
    public function acquistaBiglietto(){
        if(isset($_POST['acquista'])){
            $query = "UPDATE evento SET capienza = capienza -1 where nome = 'Padova';";
            $queryResult = mysqli_query($this->connection, $query) or die("Errore in acquistaBiglietto:" . mysqli_error($this->connection));
            $query2 = "INSERT INTO biglietto(evento,utente,data_Acquisto) VALUES (1,'allelle@gmail.com','2022.05-06');";
            $queryResult = mysqli_query($this->connection, $query2) or die("Errore in acquistaBiglietto:" . mysqli_error($this->connection));
    
            header("Location:GrazieAcquisto.php");
}
}
}
